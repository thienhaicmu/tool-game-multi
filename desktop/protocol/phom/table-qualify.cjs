...
function freeSlotsOf(c) {
  if (!c || c.Mu == null || c.uC == null || c.Mu === '' || c.uC === '') return null;
  const Mu = Number(c && c.Mu);
  const uC = Number(c && c.uC);
  if (!Number.isInteger(Mu) || Mu <= 0 || !Number.isInteger(uC) || uC < 0) return null;
  return Mu - uC;
}
...
function qualifyTable(c, opts = {}) {
  ...
  if (!Number.isInteger(Number(c.rid)) || Number(c.rid) <= 0) return { ok: false, reason: 'INVALID_RID', freeSlots: null };
  ...
  // Test D: Phom#0..Phom#13 are stake channels, even when uC <= Mu.
  // Their occupancy dropping to zero does not turn a channel into a table.
  if (/^Phom#\d+$/.test(String(c.rn || ''))) return { ok: false, reason: 'INVALID_STRUCTURE', freeSlots: freeSlotsOf(c) };
  ...
  if (opts.emptyOnly && Number(c.uC) !== 0) return { ok: false, reason: 'TABLE_NOT_EMPTY', freeSlots: free };
