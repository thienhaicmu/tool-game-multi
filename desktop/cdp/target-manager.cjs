 ...
 *
 * One TargetManager per BrowserRun (its endpoint is that run's UNIQUE Chrome
 * remote-debugging port), so target ownership never crosses runs: targetIds are
 * globally unique and the owning run is resolved via BrowserRunManager's
 * targetId -> runId index.
 *
 * Discovery works uniformly for Chrome, WebView2, CEF and Android WebView because
 * they all expose the same /json list. New/destroyed pages are detected by a
