...
// `pred` may be async: it MUST be awaited. A bare call returns an always-truthy Promise, so the loop returned
// after one immediate check and never retried — turning every async assertion into a race against Chrome.
async function waitFor(pred, ms = 6000) { const end = Date.now() + ms; while (Date.now() < end) { const v = await pred(); if (v) return v; await sleep(80); } return null; }
...
test('WU3 replay: WebView-context modified body, HTTP Direct, immutability, history, target-destroyed', async (t) => {
  ...
  const cdpPort = 9540 + (process.pid % 100);
